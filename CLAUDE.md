...
- Rollup withdrawals (OP Stack → Ethereum Sepolia) live outside the executor: `apps/web/src/lib/op-withdrawals.ts` (viem `op-stack` actions, addresses from viem's chain definitions) plus `components/withdrawals-panel.tsx` in Activity, with the list persisted as `withdrawals` in the store. Never model a multi-day wait as a route step. `scripts/withdraw.ts` does the same from the CLI (`status` / `prove` / `finalize`).
- `CHAIN_EXITS` (registry `bridges.ts`) carries the manual way off chains with no live route out (GIWA: rollup withdrawal, ~7 days); `NoRouteRow` shows it. Add an entry only with a verified bridge URL and the documented timing, never a guess.
- Own gas faucet: `DRIP_CHAINS` (registry `drip.ts`) + `FAUCET_AMOUNTS` env; server logic only in `apps/web/src/lib/server/faucet.ts` (`server-only`), route `/api/faucet`. Order: Turnstile verify → recipient balance < drip → faucet balance ≥ drip + 2×transfer gas → SET NX per address and per hashed IP (24 h) → daily cap INCR → per-chain send lock → send; any failure after reserving rolls the keys back. Memory store is dev-only; production needs Upstash/KV. The key never leaves that module. `DripFaucet` renders nothing until the faucet is configured AND a chain has enough balance, so it is currently hidden in production.
- Wallet chain: `createWagmiSigner` reads the chain from the connector (`eth_chainId`), not from wagmi's config state — wagmi reports a configured chain while the wallet sits on an unconfigured one (mainnet), so the switch was skipped and viem threw a chain mismatch. Every switch is confirmed against the provider, and `sendTransaction` re-checks right before signing.
- `/docs` and `/how-it-works` are user-facing: plain language, no error codes as headings, no CLI or contributor instructions (those live in this file and the README).
- `?watch=0x…` is handled app-wide by `WatchAddressLink` in the layout (read-only; a connected wallet wins). `WatchSwitcher` (Router, Balances, Swap) swaps the watched address inline or stops; it renders nothing while a wallet is connected. The app ships no screenshots.
- On retry, an edge whose steps failed with SLIPPAGE_EXCEEDED / QUOTE_EXPIRED / SIMULATION_FAILED / INSUFFICIENT_BALANCE is re-quoted and its unsent steps rebuilt; steps that already have a transaction on-chain are kept.
- `checkSourceBalance` runs before the first hop is built (ERC-20 inputs), and an STF-style revert is diagnosed with a balance + allowance read: `INSUFFICIENT_BALANCE` (stale plan) or `APPROVAL_MISSING` (allowance gone), never a bare "simulation failed".
...
- `TxStep.nonce`/`startBlock` are recorded before every signature. On retry, `guardDuplicate` compares the wallet's latest nonce: moved → for APPROVE steps the allowance decides (covered → step completed, short → approve again, never POSSIBLE_DUPLICATE: approvals are idempotent), otherwise `provider.recover()` (CCTP scans `DepositForBurn` logs) or, for CLAIM steps, `provider.status()`; unresolved → `POSSIBLE_DUPLICATE` and the route page's resolver (paste hash / mark unrelated). Never bypass this for burns.
