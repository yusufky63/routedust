...
- `?watch=0x…` is handled app-wide by `WatchAddressLink` in the layout (read-only; a connected wallet wins). `WatchSwitcher` (Router, Balances, Swap) swaps the watched address inline or stops; it renders nothing while a wallet is connected.
